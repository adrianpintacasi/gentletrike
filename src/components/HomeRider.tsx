...
export const HomeRider: React.FC<HomeRiderProps> = ({ driver, today, position }) => {
  ...
  return (
    <div className="overflow-hidden rounded-card bg-trust-slate text-cream-50 shadow-lg border border-cream-400/20">
      <div className="flex items-stretch">
        {/* The one number worth a glance. Everything else is context for it. */}
        <div className="flex min-w-0 flex-1 flex-col justify-center px-4 py-3">
          <p className="text-[10px] font-sans font-bold uppercase tracking-[0.14em] text-cream-400">
            Earned today
          </p>
          <p className="mt-0.5 text-3xl font-display font-extrabold leading-none tracking-tight text-trike-gold tabular-nums">
            ₱{earnings}
          </p>
        </div>
        ...
        <div className="flex shrink-0 flex-col justify-center gap-1.5 px-4 py-3">
          <span className="flex items-center gap-1.5 text-[11px] font-sans font-semibold text-cream-200 tabular-nums">
            <Route className="h-3.5 w-3.5 shrink-0 text-trike-gold" />
            {trips} trips · {distanceKm} km
          </span>
          <span className="flex items-center gap-1.5 text-[11px] font-sans font-semibold text-cream-200 tabular-nums">
            <Star className="h-3.5 w-3.5 shrink-0 text-trike-gold" />
            {driver.rating} rating
          </span>
          {weather && (
            <span className="flex items-center gap-1.5 text-[11px] font-sans font-semibold text-cream-200 tabular-nums">
              <CloudRain className="h-3.5 w-3.5 shrink-0 text-trike-gold" />
      ...
      <div className="flex items-center gap-2 border-t border-white/10 px-4 py-2">
        ...
        <p className="min-w-0 flex-1 truncate text-[11px] font-sans font-semibold text-cream-300">
          {vehicle.title} · {driver.unitNumber}
        </p>
        {sky && (
          <span className="shrink-0 text-[11px] font-sans font-semibold text-cream-400">{sky.label}</span>
        )}
      </div>
    </div>
  );
