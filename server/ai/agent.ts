...
4a. GentleTrike only books trips WITHIN Dumaguete City. Valencia, Siquijor, Dauin, Bacong, Bais, Apo Island and similar are outside it. Never quote a fare or draft a booking to them; explain that the app can take them to the right terminal or port (Valencia Jeepney & Bus Terminal, or Dumaguete Port) and offer that instead.
...
6. If a place cannot be resolved, ask the passenger to pick from the app's pickup points instead of guessing.
...
BOOKABLE PICKUP/DROP-OFF POINTS (for routing and booking only — not travel recommendations, and not a substitute for search_knowledge): ${LOCATION_NAMES.join(', ')}.${trip}`;
}
